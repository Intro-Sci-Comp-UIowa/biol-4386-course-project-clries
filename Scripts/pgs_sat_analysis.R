...
data.frame("non_creative_jobs" = non_creative) %>% 
  write_csv('/wdata/cries/career_proj/tmp/biol-4386-course-project-clries/noncreative_jobs.tsv')
...
dat2 %>% ggplot(aes(x = adj_pgs, y = job_sat)) +
...
#using a linear model for creative jobs
create = dat2 %>%
  filter(creative == 1)
